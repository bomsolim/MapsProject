...
def find_centroid(cluster):
    ...
    lo = mean([restaurant_location(i)[1] for i in cluster])
    return [lat, lo]
    # END Question 5
...
def find_predictor(user, restaurants, feature_fn):
    ...
    sxy = sum([(ls[0] * ls[1]) for ls in zip([x - mean(xs) for x in xs], [y - mean(ys) for y in ys])])
...
def best_predictor(user, restaurants, feature_fns):
    ...
    # BEGIN Question 8
    predictor = {find_predictor(user,reviewed,feature)[0]:find_predictor(user,reviewed,feature)[1] for feature in feature_fns}
    return max(predictor, key=predictor.get)
    # END Question 8
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    # BEGIN Question 9
    rating = lambda r: user_rating(user, restaurant_name(r)) \
